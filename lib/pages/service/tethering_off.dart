  ...
  Future<int> _getBattery() async {
    ...
    for (var service in services) {
      if (service.uuid.toString() == "a9d158bb-9007-4fe3-b5d2-d3696a3eb067") {
        ...
        final mixPhase3TmpHash =
            sha256.convert(utf8.encode(mixPhase3Tmp)).toString();
        final mixPhase3 = mixPhase3TmpHash + preSharedHashKey;
        final accessToken = sha256.convert(utf8.encode(mixPhase3)).toString();
        print("AccessToken: " + accessToken);

        // tethering on process
        final plainText = DateTime.now().toIso8601String() +
            "," +
            accessToken +
            "," +
            "tethering_off";
        ...
        for (var characteristic in service.characteristics) {
          if (characteristic.properties.write) {
            await characteristic.write(("#" + encrypted).codeUnits);
            break;
          }
        }
        var decrypted = decryptText(iv, preSharedKey, encrypted);
